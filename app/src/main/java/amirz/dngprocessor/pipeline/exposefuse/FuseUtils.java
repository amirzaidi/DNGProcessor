...
public class FuseUtils {
    public static Texture downsample2x(GLPrograms converter, Texture in) {
        Texture downsampled = TexturePool.get(in.getWidth() / 2 + 1,
                in.getHeight() / 2 + 1,
                in.getChannels(),
                in.getFormat());

        try (Texture tmp2 = TexturePool.get(in)) {
            try (Texture tmp = TexturePool.get(in)) {
    ...
    public static Texture upsample2x(GLPrograms converter, Texture in, Texture dimens) {
        ...
        try (Texture tmp = TexturePool.get(upsampled)) {
